...
export const FAQSection = ({ darkMode }) => {
  ...
  return (
    <section id="faq" className={`py-16 px-4 transition-colors duration-300 ${
      darkMode ? 'bg-slate-900 border-t border-slate-800' : 'bg-slate-50 border-t border-slate-100'
    }`}>
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="text-center space-y-3">
          ...
          <h2 className={`text-3xl sm:text-4xl font-black ${darkMode ? 'text-white' : 'text-slate-900'}`}>
            Got <span className="text-cyan-600">Questions?</span> We Have Answers
          </h2>
        ...
        <div className="relative max-w-md mx-auto">
          <input
            ...
            className={`w-full pl-9 pr-4 py-2.5 text-xs rounded-xl border focus:outline-none focus:ring-2 ${
              darkMode 
                ? 'bg-slate-950 border-slate-800 text-white focus:ring-cyan-500' 
                : 'bg-white border-slate-200 text-slate-900 focus:ring-sky-400 shadow-sm'
            }`}
          />
                ...
                <AnimatePresence>
                  {isOpen && (
                    <motion.div
                      ...
                      className={`px-4 pb-4 text-xs leading-relaxed pt-3 border-t ${
                        darkMode 
