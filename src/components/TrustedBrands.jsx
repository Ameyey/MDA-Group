...
export const TrustedBrands = ({ darkMode, onSelectBrand }) => {
  ...
  return (
    <section id="brands" className={`py-16 px-4 transition-colors duration-300 ${
      ...
      <div className="max-w-7xl mx-auto space-y-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div className="space-y-2 text-left">
            <div className="inline-flex items-center space-x-2 text-xs font-bold uppercase tracking-wider text-cyan-600">
            ...
            <h2 className={`text-3xl sm:text-4xl font-black ${darkMode ? 'text-white' : 'text-slate-900'}`}>
              Our Trusted <span className="text-cyan-600">Brands</span>
            </h2>
            <p className={`text-sm max-w-2xl ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>
              We deal in genuine fiber laser spare parts and optics compatible with world-leading cutting head and laser source manufacturers.
            </p>
          </div>

          {/* Toggle View Mode */}
          <div className={`flex items-center space-x-1 p-1 rounded-lg self-start md:self-auto border ${
        ...
        {viewMode === 'carousel' ? (
          <div className={`relative overflow-hidden py-6 rounded-2xl border shadow-inner ${
            ...
            <div className={`absolute left-0 top-0 bottom-0 w-16 z-10 pointer-events-none ${
              darkMode ? 'bg-gradient-to-r from-slate-950 to-transparent' : 'bg-gradient-to-r from-slate-900 to-transparent'
            }`}></div>
            <div className={`absolute right-0 top-0 bottom-0 w-16 z-10 pointer-events-none ${
              darkMode ? 'bg-gradient-to-l from-slate-950 to-transparent' : 'bg-gradient-to-l from-slate-900 to-transparent'
            }`}></div>
              ...
              <motion.div
                ...
                className={`p-6 rounded-2xl border flex flex-col items-center justify-center text-center cursor-pointer transition-all duration-300 relative group overflow-hidden ${
                  darkMode 
                    ...
                    : 'bg-white border-slate-200 hover:border-slate-400 text-slate-900 hover:shadow-lg'
                }`}
              >
                {/* Brand Logo */}
                ...
                <div className={`mt-3 pt-3 w-full text-center border-t ${
                ...
                <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded border ${
                    darkMode ? 'text-white bg-slate-800 border-slate-700' : 'text-slate-700 bg-slate-100 border-slate-300'
                  }`}>
                    Explore
                  </span>
                </div>
        ...
        <div className={`p-6 rounded-2xl border flex flex-col md:flex-row items-center justify-between gap-4 ${
          darkMode 
            ...
            : 'bg-gradient-to-r from-slate-50 via-white to-slate-50 border-slate-200 text-slate-900 shadow-sm'
        }`}>
          <div className="flex items-center space-x-4 text-left">
            <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${
              darkMode ? 'bg-slate-800/60 border border-slate-700' : 'bg-cyan-50 border border-cyan-200'
            }`}>
